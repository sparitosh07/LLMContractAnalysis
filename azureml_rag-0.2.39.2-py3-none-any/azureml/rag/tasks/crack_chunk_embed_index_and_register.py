...
from azureml.rag.utils.connections import get_obo_or_default_azure_credential
...
def crack_chunk_embed_index_and_register(
    ...
    if not output_index_asset_uri:
        ...
        # Debugging rslex_uri_volume_mount() on Linux platform only
    ...
    if "://" in output_index_asset_uri:
        ...
        if uri_match:
            if credential is None:
                credential = get_obo_or_default_azure_credential()
        ...
        if not register_workspace:
            run = Run.get_context()
            register_workspace = Workspace.from_config() if isinstance(run, _OfflineRun) else run.experiment.workspace
        else:
            try:
                run = Run.get_context()
            except Exception:
        ...
        if run and not isinstance(run, _OfflineRun):
            ...
            logger.info(f"Skip Registering MLIndex Asset '{args.asset_name}' while running offline.")
