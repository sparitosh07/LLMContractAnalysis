...
def validate_acs(
    ...
    acs_config["api_version"] = acs_metadata.get("apiVersion", ACS_API_VERSION)
    ...
    for retry_counter in range(MAX_RETRIES):
        try:
            ...
            if retry_counter == MAX_RETRIES - 1:  # Raise on the last retry
                activity_logger.error(
                    f"[Validate Deployments]: Failed to reach ACS after {MAX_RETRIES} retries. Exception: {traceback.format_exc()}"
                )
                raise ex
            activity_logger.info(
