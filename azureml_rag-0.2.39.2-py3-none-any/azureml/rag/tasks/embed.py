...
def create_embeddings(
    ...
    results_callback: Optional[Callable[[Dict, Any], None]] = None,
    batch_size: int = 100,
    num_workers: int = -1,
) -> int:
    """
            ...
            try:
                ...
                with rslex_uri_volume_mount(
                    ...
                    try:
                        ...
                        snapshot_path = Path(f"{mount_context.mount_point}/{timestamp_string}")
                        source_path = Path(args.output)
                        snapshot_path.mkdir(parents=True, exist_ok=True)
                        for file in source_path.iterdir():
                            if file.is_file():  # Ensure it's a file
                                shutil.copy(file, snapshot_path / file.name)
