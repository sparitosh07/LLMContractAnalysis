...
from azureml.rag.documents.cracking import crack_documents, files_to_document_source
...
def crack_and_chunk_and_embed(
    ...
    # Copy current output as a new snapshot
    timestamp_string = datetime.now().strftime("%Y%m%d-%H%M%S")
    snapshot_path = Path(f"{embeddings_container.local_path}/{timestamp_string}")
    embeddings_container.save(snapshot_path, with_metadata=True)
    logger.info(f"New snapshot {timestamp_string} created.")
...
def main(args, logger, activity_logger):
    ...
    with EmbeddingsContainer.mount_and_load(args.embeddings_container, activity_logger) as embeddings_container:
        ...
        # TODO:
        #     New EmbeddingsContainer code ideally would maintain snapshots automatically.
