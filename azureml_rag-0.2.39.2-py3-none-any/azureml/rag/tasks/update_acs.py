...
def main(args, logger, activity_logger):
    ...
    try:
        ...
        elif "Name or service not known" in exception_str:
            logger.error(
                f'The vector index can not be resolved. The provded index: {acs_config["index_name"]} may be deleted'
            )
            activity_logger.activity_info["error_classification"] = "UserError"
            activity_logger.activity_info["error"] = f"{e.__class__.__name__}: Can not resolve azure search index"
        elif (
