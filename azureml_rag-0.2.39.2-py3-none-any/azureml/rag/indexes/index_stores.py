...
class WeaviateStore(IndexStore):
    """A Weaviate index store used for uploading and deleting documents."""

    def __init__(self, weaviate_index_client):
        """Initialize the WeaviateStore."""
        super().__init__(IndexStoreType.WEAVIATE)
        self._weaviate_index_client = weaviate_index_client
    ...
    def upload_documents(self, documents: List[Any]):
        ...
        for document in documents:
            try:
                self._weaviate_index_client.data.insert(
                ...
                logger.error("Failed to insert one document into Weaviate index.", exec_info=e)
    ...
    def create_upload_payload_from_embedded_document(
        ...
        weaviate_properties = {
            field_mappings["doc_id"]: doc_id_encoded,
            field_mappings["content"]: document_data,
            field_mappings["url"]: document_source_info.get("url", ""),
            field_mappings["filename"]: document_source_info.get("filename", ""),
            field_mappings["title"]: document_source_info.get("title", document_metadata.get("title", "")),
            field_mappings["metadata"]: json.dumps(document_metadata),
        }

        embedding_field = field_mappings.get("embedding", None)
        weaviate_vector = {embedding_field: document_embeddings} if embedding_field else document_embeddings

        weaviate_doc = {
            "properties": weaviate_properties,
            "vector": weaviate_vector
        }

        return weaviate_doc, _get_document_size(weaviate_doc)
